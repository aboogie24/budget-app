...
describe('C026 onboarding helpers', () => {
  ...
  describe('household always (OB1)', () => {
    ...
    it('rethrows when household POST fails (create-fail, not invite-fail)', async () => {
      const post = jest.fn().mockRejectedValue(new Error('network'));
      await expect(
        ensureHouseholdAlways({ userId: 'u-x', post: post as any }),
      ).rejects.toThrow('network');
      expect(post).toHaveBeenCalledTimes(1);
    });

    it('throws when create response lacks household_id', async () => {
      const post = jest.fn().mockResolvedValue({ created: true });
      await expect(
        ensureHouseholdAlways({ userId: 'u-y', post: post as any }),
      ).rejects.toThrow('No household_id');
    });
  ...
  describe('onboarding_complete gate (OB4)', () => {
    ...
    it('returns incomplete when flag never becomes true (caller must not navigate)', async () => {
      const store: Record<string, string> = {
        budgetAppSession: JSON.stringify({ id: 'u-1', onboarding_complete: false }),
      };
      const storage = {
      ...
      const post = jest.fn().mockResolvedValue({ status: 'onboarding complete' });
      const get = jest.fn().mockResolvedValue({ id: 'u-1', onboarding_complete: false });
      const result = await completeOnboardingAndPersist({
        userId: 'u-1',
        post: post as any,
        get: get as any,
      ...
      expect(result.persisted).toBe(true);
      expect(JSON.parse(store.budgetAppSession).onboarding_complete).toBe(false);
    });
