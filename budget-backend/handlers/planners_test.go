...
func TestListDebts_PersonalOnly(t *testing.T) {
	...
	withMockDB(t, func(mock sqlmock.Sqlmock) {
		...
		rows := sqlmock.NewRows([]string{"id", "user_id", "household_id", "name", "balance", "original_balance", "apr", "min_payment", "due_day", "strategy", "is_shared", "source", "debt_category", "liability_type", "asset_depreciates"}).
